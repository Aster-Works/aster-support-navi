 ...
 * 唯一の読み取りプリミティブ。正式な source of truth は Supabase DB。
 * seed は緊急退避・ローカル初期データ用として、この interface を満たす。
 *
 ...
 *   - `supabase` … Supabase 制度DB（既定・本番唯一の正式データソース）
 *   - `seed`     … 型付き seed（緊急退避・ローカル初期データのみ）
 *   - `hybrid`   … DB 優先 + seed 補完（移行検証専用。本番運用では使わない）
 */
