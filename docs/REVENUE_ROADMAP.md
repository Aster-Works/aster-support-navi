...
## Phase 2（31〜60日）住まい・生活困窮カテゴリ強化 — ✅ コンテンツ実装

- [x] **住まい・生活困窮ガイド 8本**（`app/data/guides.ts`）。研究→敵対検証WFで作成、出典は全件 HTTP 200・
      `.go.jp`（厚労省/国交省）、禁止表現ゼロ（`safety.test` 緑）。
      生活困窮(5): 住居確保給付金（家賃が払えないとき）/ 自立相談支援機関とは / 生活福祉資金貸付 /
      生活保護のきほん（相談・申請の権利）/ 求職者支援制度。住まい(3): 公営住宅のきほん / 住宅セーフティネット / 居住支援。
      `hardship`/`housing` 生活イベントでクラスタ化、関連制度・比較リンク（hardship→/compare/livelihood、housing→/compare/housing）。
- [x] **23区・政令市 × 住居確保給付金まとめ**＝既存 `/compare/livelihood`・`/compare/housing` が全自治体横断で集約。
- [x] **支援者向け「生活困窮相談パック」サンプル**＝`/pro/samples/livelihood-housing`（既存）。
- [ ] **手動営業の開始**（Jimi）: 教会・子ども食堂・地域支援団体へ。営業文 =
