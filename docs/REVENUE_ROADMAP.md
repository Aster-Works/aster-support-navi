...
## Phase 1（15〜30日）ひとり親カテゴリ集中強化 — ⬜ 次の主戦場

最初の想定顧客を「ひとり親支援に関わる人」に絞る。

- [ ] ひとり親ガイド 10本（`app/data/guides.ts` に追加。既存 `single-parent-support` を起点に拡張）。
- [ ] 東京23区 × ひとり親支援まとめ（`/compare/single-parent` を強化 or 区別まとめページ）。
- [ ] 政令市 × ひとり親支援まとめ。
- [ ] ひとり親向け申請前チェックリスト（`/pro/samples/single-parent` を起点・既存）。
- [ ] 制度ページ→ガイド→サンプルパックの内部リンク網。
